...
def query(student_id, host):
    url = 'http://%s/kb/WebKb/Kbcx.aspx' % host
    ...
    data['__VIEWSTATE'] = form_1['__VIEWSTATE']
    data['__EVENTVALIDATION'] = form_1['__EVENTVALIDATION']
    data['txtStudntID'] = student_id
    resp = requests.post(url, data)

    courses_data = _parse(_match(resp.text))
    courses = []
